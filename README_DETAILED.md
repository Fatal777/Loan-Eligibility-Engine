...
│  URL: https://loan-eligibility-frontend.vercel.app                   │
└──────────────────────────────────────────────────────────────────────┘
                               ↓
TIER 2: API LAYER (AWS)
┌──────────────────────────────────────────────────────────────────────┐
│  API Gateway + Lambda (AWS ap-south-1)                               │
│                                                                      │
│  Endpoint: https://c41a2ucawd.execute-api.ap-south-1.../dev         │
│                                                                      │
│  4 Lambda Functions:                                                 │
│  ├─ getUploadUrl: Generate S3 presigned URL                         │
│  ├─ processCSV: Handle CSV upload trigger                           │
│  ├─ triggerMatching: Start matching workflow                        │
...
│  (CSV Uploads)                │  │  (User & Match Data)             │
│                               │  │                                  │
│  - Raw CSV files              │  │  Tables:                         │
│  - Organized by upload time   │  │  ├─ users (10,000+)             │
│  - Triggers Lambda on new file│  │  ├─ loan_products (8)           │
│  - Bucket: loan-eligibility.. │  │  ├─ matches (25,610+)           │
│                               │  │  └─ batches                     │
└───────────────────────────────┘  └──────────────────────────────────┘
                        ↓
TIER 4: AUTOMATION (n8n Docker)
┌──────────────────────────────────────────────────────────────────────┐
│  n8n Workflow Automation (Self-Hosted)                               │
│                                                                      │
│  ┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐ │
│  │  WORKFLOW A      │   │  WORKFLOW B      │   │  WORKFLOW C      │ │
│  │  Web Crawler     │──▶│  User Matching   │──▶│  Notifications   │ │
│  │                  │   │  (Rule + AI)     │   │  (AWS SES)       │ │
│  ├──────────────────┤   ├──────────────────┤   ├──────────────────┤ │
│  │ Triggers:        │   │ Triggers:        │   │ Triggers:        │ │
│  │ • Daily Schedule │   │ • Webhook from   │   │ • Webhook from   │ │
│  │ • Manual Trigger │   │   Crawler        │   │   Matching       │ │
│  │ • Webhook (CSV)  │   │ • Manual Trigger │   │ • Manual Trigger │ │
│  │                  │   │                  │   │                  │ │
│  │ Outputs:         │   │ Outputs:         │   │ Outputs:         │ │
│  │ • 8 Products     │   │ • 25,610+ Matches│   │ • Emails Sent    │ │
│  │ • Stores in DB   │   │ • AI Decisions   │   │ • Error Logs     │ │
│  └──────────────────┘   └──────────────────┘   └──────────────────┘ │
│                                                                      │
...
│                                                                      │
│  From: saadilkal.10@gmail.com                                        │
│  To: fatal.og777@gmail.com (verified)                               │
│  Status: Sandbox Mode (ready for production)                        │
│  Template: Personalized HTML with match details                     │
│  Rate: Unlimited in sandbox (25k+ emails/day)                       │
└──────────────────────────────────────────────────────────────────────┘
```
